        ...
        <div className="bg-orange-50 dark:bg-orange-950/20 p-4 rounded-lg border border-orange-200 dark:border-orange-800">
          <div className="flex items-center gap-2 text-orange-800 dark:text-orange-400">
            ...
            <span className="font-medium">
              {upcomingHarvests.length} planting(s) ready for harvest soon
            </span>
          </div>
        </div>
      )}
        ...
        {upcomingHarvests.map(h => (
          <Card 
            ...
            className={`hover:shadow-md transition-shadow border-l-4 ${
              h.daysUntilHarvest <= 3 ? "border-red-500" : "border-orange-500"
            }`}
          >
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <CardDescription className="text-sm mt-1">{h.variety}</CardDescription>
                </div>
                <Badge 
                  variant="outline" 
                  className={h.daysUntilHarvest <= 3 ? "text-red-600 border-red-600" : "text-orange-600 border-orange-600"}
                >
                  {h.daysUntilHarvest === 0 ? "Today" : `${h.daysUntilHarvest}d`}
                </Badge>
              </div>
            </CardHeader>
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{formatNumber(h.quantity)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-400">Remaining:</span>
                <span className="font-medium">{formatNumber(h.remaining_quantity ?? h.quantity)}</span>
              </div>
